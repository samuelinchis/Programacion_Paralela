...
/**
 * Mapa de color (viene del backend en C++) -> clave de textura
 * cargada en app.loader ("enemy1".."enemy5").
 *
 * Debe coincidir con el orden en que window.onload agrega los
 * assets:
 *   enemy1 -> RedStrip.png
 *   enemy2 -> BlueStrip.png
 *   enemy3 -> GreenStrip.png
 *   enemy4 -> PinkStrip.png
 *   enemy5 -> WhiteStrip.png
 *
 * Y con los nombres que manda el servidor (colorNames[] en C++):
 *   "red", "blue", "green", "pink", "white"
 */
const COLOR_TO_TEXTURE_KEY = {
  red: 'enemy1',
  blue: 'enemy2',
  green: 'enemy3',
  pink: 'enemy4',
 ...
 * Enemy Car class
 *
 * Antes elegía un sprite al azar entre 5. Ahora el color lo
 * decide el backend en C++ (cada color tiene su propio hilo
 * allá), así que aquí solo traducimos ese color a la textura
 * correspondiente. Si por algún motivo llega un color
 * desconocido o vacío, se usa un sprite aleatorio como respaldo.
 */
class EnemyCar extends Car {
  constructor(app, boundLeft, boundRight, boundBottom, speed, color) {
    const textureKey =
      COLOR_TO_TEXTURE_KEY[color] || `enemy${randomBetween(1, 5)}`;

    const texture = app.loader.resources[textureKey].texture;

    super(app, texture, boundLeft, boundRight, boundBottom, speed);

    // Guardamos el color por si se necesita más adelante
    // (debug, lógica de puntaje por color, etc.)
