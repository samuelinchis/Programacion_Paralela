...
window.onload = function () {
  ...
  function startGame() {
    // El backend ya no responde con un único JSON: transmite
    // el estado del juego como un stream de Server-Sent Events
    // (sin WebSockets). EventSource mantiene la conexión abierta
    // y dispara "onmessage" cada vez que el servidor envía un
    // evento nuevo (ver SSE_INTERVAL_MS en server.cpp).
    const backendStream = new EventSource('http://localhost:5000');

    backendStream.onmessage = (event) => {
      try {
        serverEnemies = JSON.parse(event.data);
      } catch (error) {
        console.error('Error parseando evento SSE:', error);
      }
    };

    backendStream.onerror = (error) => {
      console.error('Error en la conexión SSE con el backend:', error);
    };
